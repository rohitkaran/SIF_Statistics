...
| `index.html` | Single-file dashboard / homepage (served at `/`). Metrics block is unit-tested (`==METRICS==` markers → run under node). |
...
- `index.html` IS the homepage (committed, served at `/`). Hosting is now host-agnostic (no build
  step): GitHub Pages or Cloudflare Pages both serve `/` → `index.html`. See "Custom domain" below.
